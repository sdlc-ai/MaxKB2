...
def generate_related_by_document_id(document_id, model_id, model_params_setting, prompt, state_list=None):
    ...
    try:
        ...
        porsche_logger.error(f'根据文档生成问题:{document_id}出现错误{str(e)}{traceback.format_exc()}')
        porsche_logger.error(_('Generate issue based on document: {document_id} error {error}{traceback}').format(
        ...
        porsche_logger.info(_('End--->Generate problem: {document_id}').format(document_id=document_id))
